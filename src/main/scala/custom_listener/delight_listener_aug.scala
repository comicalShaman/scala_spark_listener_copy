...
class delight_listener_aug(sparkConf: SparkConf) extends SparkListener with Logging {
  ...
  private val writingConnector = metrics_logger_aug.getOrCreate(sparkConf)
  ...
  private def logEvent(event: SparkListenerEvent, flush: Boolean = false, blocking: Boolean = false): Unit = time(
    ...
    try {
      writingConnector.enqueueEvent(event, flush, blocking)
    } catch {
      case e: Exception =>
